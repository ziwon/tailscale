...
// to hand-copying bytes and deleting old if that fails.
//
// This is a workaround to /etc/resolv.conf being on a different
// filesystem to /etc in some container environments (because
// /etc/resolv.conf is a bind-mount from outside the container).
